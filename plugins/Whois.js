...
if (CON.LANG == 'SI') ADMİN_USER = '*Admin සංඛ්‍යාව:*', USER_USER = '*සාමාජිකයන් ගණන:*', TR_USER = '*තුර්කි සාමාජිකයන් ගණන:*', Hİ_USER = '*ඉන්දියානු සාමාජිකයන් ගණන:*', AZ_USER = '*Azərbaycan සාමාජිකයන් ගණන:*', SRİ_USER = '*ශ්‍රී ලාංකික සාමාජිකයන් ගණන:*', RU_USER = '*රුසියානු සාමාජිකයන් ගණන:*', USA_USER = '*USA සාමාජිකයන් ගණන:*', OTHER = '*වෙනත්:*'
...
if (CON.WORKTYPE == 'private') {
    Neotro.addCommand({ pattern: 'info$', fromMe: true, desc: Lang.PL_DESC }, async (message, match) => { 
    ...
    Neotro.addCommand({ pattern: 'info$', fromMe: false, desc: Lang.PL_DESC }, async (message, match) => { 
        if (message.jid.includes('-')) {
            var json = await message.client.groupMetadataMinimal(message.jid) 
            var code = await message.client.groupInviteCode(message.jid)
            var nwjson = await message.client.groupMetadata(message.jid) 
